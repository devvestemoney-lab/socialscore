  ...
  return (
    <Layout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 px-4 py-2 glass-panel rounded-xl border border-white/10 text-sm text-white/60">
              ...
              {isLoading
                ...
                : filtered.map((log) => (
                    <tr key={log.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="p-4 text-xs text-white/50 font-mono whitespace-nowrap">
                        {format(new Date(log.createdAt), 'MMM d, yyyy HH:mm:ss')}
                      </td>
                      <td className="p-4">
                        <span className={cn(
                          'px-2.5 py-1 rounded-full text-xs font-medium border whitespace-nowrap',
                          ACTION_COLORS[log.action] ?? 'text-white/60 bg-white/5 border-white/10'
                        )}>
                          {log.action}
                        </span>
                      </td>
                      <td className="p-4 font-mono text-sm text-white/80">
                        {log.targetNrc ?? <span className="text-white/30">—</span>}
                      </td>
                      <td className="p-4 text-sm">
                        <div className="text-white/70 truncate max-w-[120px]">{log.userId}</div>
                        {log.tenantId && (
                          <div className="text-xs text-white/30 truncate max-w-[120px] font-mono mt-0.5">
                            {log.tenantId}
                          </div>
                        )}
                      </td>
                      ...
                      <td className="p-4 text-xs text-white/40 max-w-[200px]">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    p === page ? 'bg-cyan-500 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10'
                  )}
                >
                  {p}
                </button>
